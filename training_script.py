...
from models.model_foundation.model_foundation_local_rev2 import get_phisat2_model, pretrained_phisat2_downstream
...
def get_models_pretrained(model_name, input_channels, output_channels, input_size, path_model_weights=None, freeze=False, device='cuda'):
    ...
    if model_name == 'phisat2_Classifier':
        ...
        print(core_kwargs)
        model = pretrained_phisat2_downstream(checkpoint_path=path_model_weights, freeze_body=freeze, task='classification',
                                              output_dim_fm = input_channels, output_dim_dw = output_channels, **core_kwargs)
        model(test_input)
        return model
    ...
    if model_name == 'phisat2_Segmentation':
        ...
        core_kwargs.pop('output_dim')
        print(core_kwargs)
        model = pretrained_phisat2_downstream(checkpoint_path=path_model_weights, freeze_body=freeze, task='segmentation',
                                              output_dim_fm = input_channels, output_dim_dw = output_channels, **core_kwargs)
        model(test_input)
        return model
