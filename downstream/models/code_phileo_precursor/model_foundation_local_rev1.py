...
class Foundation(nn.Module):
    ...
    def forward(self, x):
        x = self.stem(x)
        embeddings, skips, (out_coords, out_clouds, out_water, out_buildings, out_landcover) = self.encoder(x)
        decoded = self.decoder(embeddings, skips)
        reconstruction = self.head(decoded)

        return reconstruction, embeddings, (out_coords, out_clouds, out_water, out_buildings, out_landcover)
