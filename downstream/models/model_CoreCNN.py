...
# from models.model_foundation.model_foundation_local_rev2 import FoundationEncoder, FoundationDecoder
...
class GeolocationUnet(CoreUnet):
    def __init__(self, *,
                 input_dim=10,
                 output_dim=3,  # 3 for latitude, sin(longitude), cos(longitude)
                 depths=None,
                 dims=None,
                 activation="relu",
                 norm="batch",
                 padding="same",
                 **kwargs):
        super(GeolocationUnet, self).__init__(
            input_dim=input_dim,
            output_dim=output_dim,  # Temporarily set to 3
            depths=depths,
            dims=dims,
        ...
        # Remove the original head intended for segmentation
        self.head = None  # Or you can leave it as is since we'll override forward

        # Add a new head for geolocation prediction
        # self.geolocation_head = nn.Sequential(
        #     nn.AdaptiveAvgPool2d((1, 1)),  # Global Average Pooling
        #     nn.Flatten(),                  # Flatten to [batch_size, channels]
        #     nn.Linear(self.dims[0], 128), # Optional: Add a hidden layer
        #     nn.ReLU(),
        #     nn.Linear(128, output_dim)     # Output layer: 3 values
        # )
        self.geolocation_head = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),  # Global Average Pooling to [batch_size, channels, 1, 1]
            nn.Flatten(),                  # Flatten to [batch_size, channels]
            nn.Linear(self.dims[0], output_dim)  # Single linear layer to output_dim
        )

    def forward(self, x):
        # Utilize the original forward_body method to extract features
        x = self.forward_body(x)
        x = self.geolocation_head(x)
