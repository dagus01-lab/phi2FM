...
class CoreEncoderGeoPretrained(nn.Module):
    ...
    def load_encoder_weights(self,checkpoint, unet):
        ...
        for k, name in zip(checkpoint.keys(),model_sd.keys()):
            if k.startswith('head') or k.startswith('geolocation_head'):
                continue
            v = checkpoint[k]
...
class CoreEncoderGeoPretrainedFM4T(nn.Module):
    def __init__(self,
                output_dim,
                checkpoint,
                core_encoder_kwargs,
        ...
        self.freeze_body = freeze_body
        super(CoreEncoderGeoPretrainedFM4T, self).__init__()
    ...
    def rename_key(self, ckpt_key):
        # Example: "stem.squeeze.excitation.0.weight" -> "stem.0.squeeze.excitation.0.weight"
        if ckpt_key.startswith("stem."):
            # Insert a ".0." after "stem"
            return ckpt_key.replace("stem.", "stem.0.")
        
        # Example: "encoder.downsample.0.0.weight" 
        # might need to become "encoder_blocks.0.blocks.0.conv1.weight"
        if ckpt_key.startswith("encoder."):
            # You might parse "encoder.downsample.X.Y" -> "encoder_blocks.X.blocks.Y"
            # or "encoder.blocks_down.X.Y" -> "encoder_blocks.X.blocks.Y"
            # The actual logic depends on how your architecture changed
            # This is just a conceptual example:
            ckpt_key = ckpt_key.replace("encoder.blocks_down", "encoder_blocks")
            # if you need to do more fine-grained replacements, do them here

        # Return the newly transformed key
        return ckpt_key

    def load_encoder_weights(self, checkpoint, unet):
        model_sd = unet.state_dict()            # downstream model's state_dict
        shared_weights = OrderedDict()
        ...
        # checkpoint is a dict: {param_name: param_value, ...}
        for ckpt_key, ckpt_value in checkpoint.items():
            # print(f"Trying {ckpt_key} from checkpoint -- shape: {ckpt_value.size()}")
            # Skip if it belongs to any of the heads or decoder
            if ckpt_key.startswith("head") \
            or ckpt_key.startswith("decoder") \
            or ckpt_key.startswith("geolocation_head"):
                continue
            
            renamed_key = self.rename_key(ckpt_key)
            # If this ckpt_key exists in the model, check shape match
            if renamed_key in model_sd:
                # Ensure shapes match
                if ckpt_value.size() == model_sd[renamed_key].size():
                    shared_weights[renamed_key] = ckpt_value
                    print(f"Loading {ckpt_key} (-> {renamed_key}) from checkpoint -- shape: {ckpt_value.size()}")
                else:
                    # Use renamed_key in both the message and the dictionary lookup
                    raise ValueError(
                        f"Mismatched shape for layer '{ckpt_key}' in checkpoint vs. '{renamed_key}' in pretrained model: "
                        f"{ckpt_value.size()} vs {model_sd[renamed_key].size()}"
                    )

        # Update model_sd with shared_weights
        model_sd.update(shared_weights)
...
def get_core_encoder_kwargs(output_dim, input_dim, core_size, full_unet=True, **kwargs):
    ...
    if core_size=='core_nano':
        core_kwargs['encoder_depths'] = [2, 2, 8, 2]
        core_kwargs['decoder_depths'] = [2, 2, 2, 2] if full_unet else [2, 2, 8, 2]
        core_kwargs['dims']=[80, 160, 320, 640]

    elif core_size=='core_tiny':
        core_kwargs['encoder_depths']=[3, 3, 9, 3]
        core_kwargs['decoder_depths'] = [3, 3, 3, 3] if full_unet else [3, 3, 9, 3]
        core_kwargs['dims']=[96, 192, 384, 768]

    elif core_size=='core_base':
        core_kwargs['encoder_depths']=[3, 3, 27, 3]
        core_kwargs['decoder_depths'] = [3, 3, 3, 3] if full_unet else [3, 3, 27, 3]
