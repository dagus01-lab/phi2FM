...
class FloodPredictorHSL(nn.Module):
    def __init__(
        ...
        self.foundation_spectral = FloodPredictorSpectral(input_dim=5, output_dim=32, path_weights=path_weights)
        self.foundation_static = FloodPredictorStatic(input_dim=7, output_dim=32)

        self.head = nn.Sequential(
            nn.Dropout2d(0.2),
            CNNBlock(64, 32, activation=nn.LeakyReLU(), residual=False, chw=[self.input_dim, 128, 128]),
            CNNBlock(32, 2, activation=nn.LeakyReLU(), residual=False, chw=[self.input_dim, 128, 128]),
        )
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss = spatial_loss(outputs, labels)
                    # loss = SoftSpatialSegmentationLossSimple()
