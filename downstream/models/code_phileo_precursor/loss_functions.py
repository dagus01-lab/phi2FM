...
def cossim_loss(embeddings, embeddings_aug, alpha=0.5):
    ...
    loss_cos_negative = F.cosine_embedding_loss(torch.flip(embeddings, dims=(0,)), embeddings_aug, torch.zeros(embeddings.shape[0]).cuda())

    euclidian_distance = torch.sqrt(torch.sum((embeddings - embeddings_aug) ** 2, dim=1))

    return loss_cos_positive * (1 + alpha) + loss_cos_negative * (1 - alpha) + euclidian_distance.mean()
