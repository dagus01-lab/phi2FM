...
class SCLCloudSnowTask(EOTask):
    def __init__(self, scl_feature: Tuple[FeatureType, str]):
        ...
        self.scl_cloud_shadow_feature = (FeatureType.MASK, "SCL_CLOUD_SHADOW")
        self.scl_cirrus_feature = (FeatureType.MASK, "SCL_CIRRUS")
        self.scl_snow_feature = (FeatureType.MASK, "SCL_SNOW")
    ...
    def execute(self, eopatch: EOPatch) -> EOPatch:
        ...
        eopatch[self.scl_cloud_shadow_feature] = (scl == 3).astype(np.uint8)
        eopatch[self.scl_cirrus_feature] = (scl == 10).astype(np.uint8)
        eopatch[self.scl_snow_feature] = (scl == 11).astype(np.uint8)
        del eopatch[self.scl_feature]
...
class AddMetadataTask(EOTask):
    ...
    @staticmethod
    def filter_and_sort_tiles(tiles: List[Dict], timestamps: List[datetime], use_margin: bool = False) -> List[Dict]:
        filtered_tiles = []
        available_dates = []
        for tile in tiles:
            tile_dt = parse_time(tile["properties"]["datetime"], ignoretz=True)
            if use_margin:
                # Check if the tile's timestamp is within one day of any target timestamp
                is_within_margin = any((timestamp - timedelta(days=1) <= tile_dt <= timestamp + timedelta(days=1)) for timestamp in timestamps)
            else:
                # Check if the tile's timestamp matches exactly with any target timestamp
                is_within_margin = any(timestamp.date() == tile_dt.date() for timestamp in timestamps)

            if is_within_margin:
                if tile_dt.date() not in available_dates:
    ...
    def execute(self, eopatch: EOPatch, use_margin: bool = False, **kwargs) -> EOPatch:
        ...
        query = catalog.search(
            ...
            bbox=eopatch.bbox,
            time=[eopatch.timestamp[0] - timedelta(days=1), eopatch.timestamp[-1] + timedelta(days=1)],
        )
        tiles = self.filter_and_sort_tiles(list(query), eopatch.timestamp, use_margin)
        ...
        if False:
            eopatch.scalar["earth_sun_dist"] = np.ones((dim, 1)) * 0.96934043
        ...
        for ts_idx, eop_ts in enumerate(eopatch[self.input_feature]):
            ...
            for b_idx in range(eop_ts.shape[-1]):
                ...
                band = warpAffine(
                    ...
                    dsize=(eop_ts_band.shape[1], eop_ts_band.shape[0]),
