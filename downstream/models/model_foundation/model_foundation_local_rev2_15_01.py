        ...
        *,
        ...
        for i in range(self.steps - 1):
            ...
            self.downsample.append(
                nn.Sequential(
                    nn.Conv2d(dims[i], dims[i + 1], 1, padding=0),
        ...
        # -------------------------------------------------------------
        self.block_scalers = nn.ModuleList([ScaleSkip2D(dims[i]) for i in range(self.steps)])
            ...
            for _ in range(depths[i]):
                stage_blocks.append(
                    CNNBlock(
                        channels_in=dims[i],
                        channels_out=dims[i],
                        chw=None if norm=='batch' else [dims[i], self.sizes[i], self.sizes[i]],
...
class FoundationDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        # -------------------------------------------------------------
        # Upsamplers
        # dims[i] -> dims[i+1] through an upsampling + conv
        # -------------------------------------------------------------
        self.upsamplers = nn.ModuleList()
        if ov_compatiblity:
            for i in range(self.steps - 1):

                # Create a fixed bilinear upsampler for current number of channels
                bilinear_upsampler = make_bilinear_upsample(self.dims[i])
                # Followed by a Conv2d to change channel dimensions and apply further processing
                conv = nn.Conv2d(
    ...
    def forward(self, bottom_feats, skips):
        """
        bottom_feats: (B, dims[-1], H_enc, W_enc) from the encoder
        ...
        for i in range(self.steps):
            ...
            # Upsample if not the final stage
            if i < self.steps - 1:
...
class FoundationModel4Task(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.head_seg = CNNBlock(
            channels_in=self.dims[0],
            channels_out=31,
            chw=[31, self.img_size, self.img_size],
            activation=self.activation,
    ...
    def forward(self, x, print_outs = False):
        ...
        # Reconstruction
        reconstruction = self.head_recon(decoded_feats)  # (B, 3, 128, 128)
        ...
        if print_outs: 
            outputs = [
                ...
                ("geolocation out", geo_pred.shape, geo_pred.mean(), geo_pred.std(), geo_pred.min(), geo_pred.max()),
...
class MultiTaskLoss(nn.Module):
    def __init__(self, apply_zoom=False):
        ...
        # Weights for Climate Segmentation
        # Counts for each class as provided
    ...
    def forward(self, output, labels):
        ...
        self.log_sigma_recon.data.clamp_(min=-3, max=1)
        self.log_sigma_perc.data.clamp_(min=-3, max=1)
        self.log_sigma_seg.data.clamp_(min=-3, max=1)
        self.log_sigma_geo.data.clamp_(min=-3, max=1)
        self.log_sigma_tv.data.clamp_(min=-3, max=1)

        if self.apply_zoom:
            self.log_sigma_zoom.data.clamp_(min=-3, max=1)
